...
exports.getDashboardSummary = async (req, res) => {
  try {
    ...
    const pendingQueries = await Query.countDocuments({ ...divisionFilter, status: 'Pending' });
    const inProgressQueries = await Query.countDocuments({ ...divisionFilter, status: 'In Progress' });
    const resolvedQueries = await Query.countDocuments({ ...divisionFilter, status: 'Resolved' });
    const rejectedQueries = await Query.countDocuments({ ...divisionFilter, status: 'Rejected' });
  ...
  try {
    ...
    const performanceData = await Promise.all(
      divisions.map(async (division) => {
        const totalQueries = await Query.countDocuments({ division: division._id });
        ...
        const recentResolutionRate = recentQueries > 0 ? (recentResolved / recentQueries * 100).toFixed(1) : 0;
